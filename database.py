...
engine = create_engine('postgresql://postgres:Maryam@1369@localhost:5432/googlenews') # database engine object from SQLAlchemy that manages connections to the database
                                                    # DATABASE_URL is an environment variable that indicates where the database lives
db = scoped_session(sessionmaker(bind=engine))    # create a 'scoped session' that ensures different users' interactions with the

f = open("googlenews_top_monthly_2019_45cities_text.csv")
reader = csv.reader(f)
